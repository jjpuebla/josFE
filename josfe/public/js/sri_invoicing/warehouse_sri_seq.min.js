  ...
  try {
    for (let doc of docsToCheck) {
      const res = await frappe.call({
        method: "josfe.sri_invoicing.core.utils.common.can_delete_pe",
        args: {
...
if (res.message === true) {
  ...
  async function callRPC(frm, row, updates, note, initiatingNow) {
    return await frappe.call({
      method: "josfe.sri_invoicing.core.numbering.state.initiate_or_edit",
      args: {
        warehouse_name: frm.doc.name,
...
// ------------------------------ Estado toggle rule -------------------------------
  frappe.ui.form.on(CHILD, {
      estado: function (frm, cdt, cdn) {
          ...
          // 🚫 New rule: cannot change estado if not initialized
          if (!parseInt(row.initiated || 0)) {
              frappe.msgprint(__('No puede activar un Punto de Emisión hasta que esté inicializado.'));
              row.estado = "Inactivo";
