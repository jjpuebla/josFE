  ...
  function lock_grid(frm) {
    ...
    if (!grid.__josfe_patched) {
      ...
      grid.refresh = function () {
        ...
        // Hide delete controls after every render
        const $w = $(this.wrapper);
        $w.find('.grid-remove-rows, .grid-delete, .grid-row .btn-remove, .grid-row .grid-delete-row').hide();
        // Intercept any residual clicks
        $w.off('click.josfe-no-del')
          .on('click.josfe-no-del', '.grid-remove-rows, .grid-delete, .grid-row .btn-remove, .grid-row .grid-delete-row', (e) => {
            e.preventDefault(); e.stopImmediatePropagation();
            frappe.msgprint(__('No elimines Puntos de Emisión. Cambia el campo <b>Estado</b> a <b>Inactivo</b>.'));
            return false;
          });
    ...
    // Hide delete controls immediately
    const $w = $(grid.wrapper);
    $w.find('.grid-remove-rows, .grid-delete, .grid-row .btn-remove, .grid-row .grid-delete-row').hide();
    $w.off('click.josfe-no-del')
      .on('click.josfe-no-del', '.grid-remove-rows, .grid-delete, .grid-row .btn-remove, .grid-row .grid-delete-row', (e) => {
